...
def main():
    ...
    tmpdir = "%s/fartcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and fartcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "fartcoind"]) is not None:
    ...
    #Set env vars
    if "FARTCOIND" not in os.environ:
        os.environ["FARTCOIND"] = build_dir + '/src/fartcoind' + exeext
        os.environ["FARTCOINCLI"] = build_dir + '/src/fartcoin-cli' + exeext
