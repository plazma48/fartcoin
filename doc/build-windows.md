...
for Ubuntu Xenial does not produce working executables for some of the Fartcoin Core applications.
...
causes two of the fartcoin executables to crash shortly after start up. The bug is related to the
...
It's not possible to build the fartcoin code using the win32 version of the Mingw-w64 cross compilers (at least not without
modifying headers in the fartcoin source code).
